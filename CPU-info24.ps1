...
# URL: https://www.tenforums.com/drivers-hardware/106331-powershell-script-cpu-information-incl-cpuid.html
...
Write-Output "`nCPU-info [Version 2.4] © 2019 Dimitri Delopoulos"
...
if (bcdedit.exe | Select-String -Pattern "winload.efi") 
    {$FirmwareType = "UEFI"}
...
if (! $UpdatedBIOSMicrocode -and ($RunningMicrocodeHEX -eq $BIOSMicrocodeHEX)) { $Disclaimer = $true }
...
$BIOSAge = ((Get-Date) - $BIOSInfo.ReleaseDate).Days
if ($FirmwareType -eq "Legacy BIOS") {
  Write-Output "`n$('Boot Mode'.PadRight(29)): $FirmwareType"
  Write-Output "$($FirmwareType) $('Version')$((':').Padleft(11)) $($BIOSInfo.Name)"
  Write-Output "$($FirmwareType) $('Manufacturer')$((':').Padleft(6)) $($BIOSInfo.Manufacturer)"
  Write-Output "$($FirmwareType) $('Serial Number')$((':').Padleft(5)) $($BIOSInfo.SerialNumber)"
  Write-Output "$($FirmwareType) $('Release Date')$((':').Padleft(6)) $(Get-Date ($BIOSInfo.ReleaseDate) -Format d) $("($BIOSAge", "days ago)")"
}
if ($FirmwareType -eq "UEFI") {
  Write-Output "`n$('Boot Mode'.PadRight(29)): $FirmwareType"
  Write-Output "$($FirmwareType) $('Version')$((':').Padleft(18)) $($BIOSInfo.Name)"
  Write-Output "$($FirmwareType) $('Manufacturer')$((':').Padleft(13)) $($BIOSInfo.Manufacturer)"
  Write-Output "$($FirmwareType) $('Serial Number')$((':').Padleft(12)) $($BIOSInfo.SerialNumber)"
  Write-Output "$($FirmwareType) $('Release Date')$((':').Padleft(13)) $(Get-Date ($BIOSInfo.ReleaseDate) -Format d) $("($BIOSAge", "days ago)")"
}
...
Remove-Variable CPU_Type, CPU_Architecture, CPU_Family, CPU_UpgradeMethod, PCModel, CIMCPU, CPUInfo,
                FirmwareType, CPURegistryPath, RunningMicrocode, RunningMicrocodeHEX, BIOSInfo, BIOSAge, 
                Disclaimer, DisclaimerText, BIOSMicrocode, BIOSMicrocodeHEX, UpdatedBIOSMicrocode, 
                MicrocodeLoader -ErrorAction SilentlyContinue
# Pause
